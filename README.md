...
CUDA 预览包会额外包含 `FileDeduperCuda.dll`。这个 provider 使用 CUDA kernel 计算完整文件 MD5；当前 v1 以正确性和可回退为主，不保证比 CPU 更快。若 CUDA runtime 不可用，程序会显示 fallback 原因并继续使用 CPU。
...
发布时会保留两个方向：轻量绿色版（无 GPU 依赖）和 CUDA/GPU 支持版（单独说明 NVIDIA driver/CUDA runtime 要求），用户按机器条件选择。
...
生成文件位于 `dist\FileDeduper-v2.2.0-preview1-lite.zip`。

打包 CUDA 预览版：

```cmd
build-gpu.cmd
package-gpu-release.cmd
```

生成文件位于 `dist\FileDeduper-v2.2.0-preview1-cuda.zip`。

## 版本更新记录

### v2.2.0-preview.1

- 新增排除关键词：文件夹关键词和文件名关键词分栏配置。
- 文件夹关键词命中目录路径或目录名时跳过整棵目录；文件名关键词只匹配文件名。
- 扫描状态显示排除文件数和排除目录数。
- 配置文件保存排除关键词，随绿色版一起迁移。
- 新增 CUDA native provider 与 CUDA 预览包；CUDA 不可用时回退 CPU。
- 当前 CUDA v1 是 correctness-first，benchmark 会如实显示性能，不宣传必然加速。
