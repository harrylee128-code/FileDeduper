...
当前 v2 版本不启用 CUDA、OpenCL 或 Intel 硬件加速。重复文件查找的主要瓶颈通常是磁盘 I/O 和安全文件操作，而不是 MD5 计算；引入 GPU 运行时也会破坏绿色便携和低依赖目标。

后续如果增加图片相似度、视频指纹或 AI 内容识别等计算密集功能，可以考虑以可选插件方式接入 DirectML、OpenCL、OpenVINO 或 ONNX Runtime GPU，不影响基础查重功能。
