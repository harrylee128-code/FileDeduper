================================================================================
                FileDeduper  ·  文件查重清理助手 v2.2 preview 1 Lite
================================================================================
...
【当前版本】
v2.2.0-preview.1 Lite

更新重点：
  ✓ 新增排除关键词：文件夹关键词 / 文件名关键词分栏配置
  ✓ 文件夹关键词命中目录路径或目录名时跳过整棵目录
  ✓ 扫描状态显示排除文件数和排除目录数
  ✓ 新增实验性 CUDA native provider 与 CUDA 预览包
  ✓ 保留哈希并行度、GPU fallback、benchmark 能力
...
✓ 排除关键词：可保护不想扫描或不想删除的目录/文件名
✓ 性能基线：可配置哈希并行度，Auto 模式适配高速磁盘
✓ 加速路线：轻量版不依赖 GPU；CUDA 实验版作为独立发布包
...
会自动扫描 TestDedupe 测试文件夹，验证：
  ...
  ✓ 永久删除功能正常
  ✓ 配置读写正常
  ✓ GPU experimental 无 provider 时安全回退 CPU
  ✓ 并行哈希不会破坏完整内容验证
...
- 架构：分层设计 (Models → Utils → Core → Forms)
  ...
  * Utils：ConfigStore (JSON), RecycleBinHelper (P/Invoke), HashHelper (MD5)
           HashEngine/HashBenchmark/HashParallelism (加速抽象与吞吐测量)
...
- 排除关键词：文件夹关键词 / 文件名关键词
- 哈希计算加速：Auto / CPU only / GPU experimental
- 哈希并行度：0=Auto，1=单线程，2-64=手动并行
- 最小文件大小：忽略小于此大小的文件
...
- 删除操作不可逆（永久删除模式会二次确认）
- 大文件哈希计算需要时间，可随时取消
- 机械硬盘、网络盘或 NAS 建议先使用单线程或 Auto 小样本测试
- CUDA 预览包是正确性优先，不保证一定比 CPU 更快
- 配置文件损坏会自动回退默认配置
