...
The benchmark reports file count, bytes processed, provider name, elapsed time, and MB/s. Use disposable data first. For real TB-scale validation, run on the target GPU machine and compare CPU-only against GPU experimental or any future native provider.
