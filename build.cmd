...
"%CSC%" /noconfig /nologo /target:winexe /platform:anycpu /langversion:5 /optimize+ /nostdlib+ /out:"%OUTDIR%\FileDeduper.exe" /win32icon:"%ROOT%Trash.ico" /reference:"%REFDIR%\mscorlib.dll" /reference:"%REFDIR%\System.dll" /reference:"%REFDIR%\System.Core.dll" /reference:"%REFDIR%\System.Drawing.dll" /reference:"%REFDIR%\System.Windows.Forms.dll" "%ROOT%Program.cs" "%ROOT%Properties\AssemblyInfo.cs" "%ROOT%Forms\MainForm.cs" "%ROOT%Forms\SettingsForm.cs" "%ROOT%Core\FileScanner.cs" "%ROOT%Core\DuplicateDetector.cs" "%ROOT%Core\SmartMarker.cs" "%ROOT%Core\FileDeleter.cs" "%ROOT%Models\Enums.cs" "%ROOT%Models\FileEntry.cs" "%ROOT%Models\DuplicateGroup.cs" "%ROOT%Models\AppSettings.cs" "%ROOT%Utils\AppVersionInfo.cs" "%ROOT%Utils\ConfigStore.cs" "%ROOT%Utils\MiniJson.cs" "%ROOT%Utils\RecycleBinHelper.cs" "%ROOT%Utils\HardwareCapabilityDetector.cs" "%ROOT%Utils\HashEngine.cs" "%ROOT%Utils\HashParallelism.cs" "%ROOT%Utils\HashBenchmark.cs" "%ROOT%Utils\HashHelper.cs"
